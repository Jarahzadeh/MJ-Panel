...
async function checkAutoResets(env, ctx) {
	...
	try {
		...
		const cacheReq = new Request("https://internal.mj/auto_reset");
...
async function checkAutoRotates(env, ctx) {
	...
	try {
		const cache = caches.default;
		const cacheReq = new Request("https://internal.mj/auto_rotate");
		if (await cache.match(cacheReq)) return;
...
const SubscriptionService = {
	async generateText(user, host) {
		...
		const links = [];
		const m1 = decodeURIComponent("%E2%9A%A0%EF%B8%8F%D9%BE%D9%86%D9%84%20MJ%20%D8%A7%D8%B3%D8%AA%E2%9A%A0%EF%B8%8F");
		const m2 = decodeURIComponent("%F0%9F%9A%80%40MJ_PANEL%20%F0%9F%9A%80");
		...
		const rawPath = "/stream/MJ_PANEL/" + (user.uuid ? user.uuid.split("-")[4] : "default");
		let proxyList = [];
		try {
	...
	const processWsMessage = async (chunk) => {
		...
		if (!isHeaderParsed) {
			...
			if (request) {
				...
				const expectedPath = "/stream/MJ_PANEL/" + (user.uuid ? user.uuid.split("-")[4] : "default");
                ...
                <div class="flex items-center gap-3 bg-gray-100 dark:bg-zinc-800/60 px-3 py-1.5 rounded-full border border-gray-200 dark:border-zinc-800/80 shadow-sm flex-shrink-0 w-fit">
                    <a href="https://wa.me/971557256658" target="_blank" rel="noopener noreferrer" class="flex items-center gap-2 px-4 py-2 rounded-full bg-green-500/20 hover:bg-green-500/30 border border-green-400/20 hover:border-green-300/30 transition-all duration-300 text-green-700 dark:text-green-300 hover:text-green-900 dark:hover:text-green-100 font-bold text-sm shadow-sm">
                        <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M20.52 3.48A11.8 11.8 0 0 0 12.05 0C5.41 0 .03 5.38 0 12.02c0 2.11.55 4.17 1.6 5.98L0 24l6.17-1.62a11.96 11.96 0 0 0 5.87 1.5h.01c6.64 0 12.02-5.38 12.02-12.02 0-3.22-1.25-6.25-3.55-8.38Z"/>
                        </svg>
                        تماس واتساپ
                    </a>
                </div>
...
<div id="free-panel-warning-modal" class="fixed inset-0 z-[85] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-rose-500/50 rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <h3 class="font-black text-xl text-gray-900 dark:text-white mb-2">خوش آمدی به MJ Panel</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6 leading-relaxed font-medium">
            خوشحالیم که اینجا هستی. برای پشتیبانی و هماهنگی فقط از واتساپ استفاده کن.
        </p>
                    ...
                    <div class="flex flex-col gap-2 pt-4 lg:pt-0 border-t-2 lg:border-t-0 border-gray-300 dark:border-amoled-border justify-between">
                        <div class="flex flex-col flex-1">
                            ...
                            <div class="grid grid-cols-2 gap-2 mb-2 w-full">
                                ...
                                <a href="https://wa.me/971557256658" target="_blank" class="text-[11px] bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2 py-2 rounded border border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition font-black shadow-sm text-center whitespace-nowrap">ساخت پـروکـسـی شخصی</a>
                            </div>
...
<div id="support-modal" class="fixed inset-0 z-[105] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-red-500/50 rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <div class="space-y-3">
            <a href="https://wa.me/971557256658" target="_blank" class="w-full py-3 bg-transparent border-2 border-green-600 text-green-700 hover:bg-green-50 dark:border-green-500/60 dark:text-green-400 dark:hover:bg-green-500/10 font-bold rounded-md text-sm transition duration-300 shadow-sm flex items-center justify-center gap-2">
                <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M20.52 3.48A11.8 11.8 0 0 0 12.05 0C5.41 0 .03 5.38 0 12.02c0 2.11.55 4.17 1.6 5.98L0 24l6.17-1.62a11.96 11.96 0 0 0 5.87 1.5h.01c6.64 0 12.02-5.38 12.02-12.02 0-3.22-1.25-6.25-3.55-8.38Z"/></svg>
                تماس واتساپ
            </a>
        </div>
...
<div id="update-modal" class="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <div class="space-y-3">
            ...
            <a href="https://wa.me/971557256658" target="_blank" class="w-full py-3.5 bg-orange-50 dark:bg-orange-950/30 hover:bg-orange-100 dark:hover:bg-orange-900/50 text-orange-600 dark:text-orange-500 border border-orange-300 dark:border-orange-500 font-bold rounded-md text-sm transition duration-300 shadow-sm flex items-center justify-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path>
                </svg>
                ارتباط واتساپ برای آپدیت
            </a>
        </div>
	...
	try {
        ...
        async function exportUsersBackup() {
            ...
            try {
                ...
                downloadAnchor.setAttribute("download", "mj_full_backup_" + dateStr + ".json");
...
<div class="flex flex-col gap-4 mt-6 z-10">
    <div class="flex flex-wrap items-center gap-3 sm:gap-4 justify-center">
        <a href="https://wa.me/971557256658" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-gray-700 dark:text-zinc-300 hover:text-sky-500 dark:hover:text-sky-400 group">
    ...
    <div class="flex flex-wrap items-center gap-3 sm:gap-4 justify-center">
        <a href="https://wa.me/971557256658" target="_blank" class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-full shadow-sm hover:shadow-md transition text-sm font-bold text-green-700 dark:text-green-300 hover:text-green-900 dark:hover:text-green-100 group">
            <svg class="w-5 h-5 text-green-500 dark:text-green-400 group-hover:scale-110 transition" viewBox="0 0 24 24" fill="currentColor">
                <path d="M20.52 3.48A11.8 11.8 0 0 0 12.05 0C5.41 0 .03 5.38 0 12.02c0 2.11.55 4.17 1.6 5.98L0 24l6.17-1.62a11.96 11.96 0 0 0 5.87 1.5h.01c6.64 0 12.02-5.38 12.02-12.02 0-3.22-1.25-6.25-3.55-8.38Z"/>
            </svg>
            تماس واتساپ
        </a>
