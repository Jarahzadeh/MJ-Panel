...
async function fetchWithFallback(path, options = {}) {
	const githubUrl = `https://raw.githubusercontent.com/Jarahzadeh/MJ-Panel/main/${path}`;
	const staticUrl = `https://cdn.jsdelivr.net/gh/Jarahzadeh/MJ-Panel@main/${path}`;
	try {
		...
		if (url.pathname === "/api/update-panel" && request.method === "POST") {
			...
			try {
				const cfHeaders = {
					...
					"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) MJPanel/1.0"
				};
				if (!currentAccountId) {
				...
				const githubRes = await fetch("https://raw.githubusercontent.com/Jarahzadeh/MJ-Panel/main/Source.js?t=" + Date.now(), {
					headers: {
				...
				formData.append("Source.js", new Blob([newCode], { type: "application/javascript+module" }), "Source.js");
...
const DbService = {
	...
	async sha256(message) {
		const salt = "MJ_PANEL_SECURE_SALT_2026";
...
const SubscriptionService = {
	async generateText(user, host) {
		...
		const dynPath = encodeURIComponent("/stream/MJ_PANEL/" + (user.uuid ? user.uuid.split("-")[4] : "default"));
		const links = [];
		const m1 = decodeURIComponent("%E2%99%A8%EF%B8%8F%20%40MJ_PANEL%20%E2%99%A8%EF%B8%8F");
		const m2 = decodeURIComponent("%E2%99%A8%EF%B8%8F%20%40MJ_PANEL%20%E2%99%A8%EF%B8%8F");
		...
		ips.forEach((ip) => {
			ports.forEach((portStr) => {
				...
				const remark = "MJ | " + flagEmoji + " | " + user.username;
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="flex flex-row flex-wrap justify-center items-center gap-3 w-full md:w-auto">
                ...
                <div class="flex items-center gap-3 bg-gray-100 dark:bg-zinc-800/60 px-3 py-1.5 rounded-full border border-gray-200 dark:border-zinc-800/80 shadow-sm flex-shrink-0 w-fit">
                    <a href="https://wa.me/971557256658" target="_blank" rel="noopener noreferrer" class="flex items-center gap-2 px-4 py-2 rounded-full bg-green-500/20 hover:bg-green-500/30 border border-green-400/20 hover:border-green-300/30 transition-all duration-300 text-green-700 dark:text-green-300 hover:text-green-900 dark:hover:text-green-100 font-bold text-sm shadow-sm">
                        <svg class="w-[22px] h-[22px] flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M19.07 4.93A10 10 0 003.41 17.78L2 22l4.35-1.36A10 10 0 1019.07 4.93zM12 20a7.98 7.98 0 01-4.08-1.12l-.29-.17-2.58.81.84-2.51-.19-.32A8 8 0 1112 20zm4.62-5.84c-.25-.13-1.47-.73-1.7-.81-.23-.08-.39-.13-.56.13s-.64.81-.78.98-.29.19-.54.06a6.58 6.58 0 01-1.94-1.2 7.24 7.24 0 01-1.35-1.67c-.14-.24-.01-.37.12-.49.12-.12.27-.29.4-.44.13-.15.17-.25.26-.42.09-.17.05-.31-.02-.44-.07-.13-.56-1.35-.77-1.85-.2-.48-.4-.41-.56-.42h-.48c-.17 0-.44.06-.67.31-.23.26-.88.86-.88 2.1 0 1.23.9 2.42 1.03 2.59.13.17 1.77 2.7 4.28 3.79.6.26 1.07.42 1.44.54.61.2 1.17.17 1.61.1.49-.07 1.47-.6 1.68-1.18.21-.58.21-1.08.15-1.18-.06-.1-.23-.17-.48-.3z"/>
                        </svg>
                        واتساپ / WhatsApp
                    </a>
                </div>
...
<div id="free-panel-warning-modal" class="fixed inset-0 z-[85] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-rose-500/50 rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6 leading-relaxed font-medium">
            این پـنـل با برند <span class="text-rose-500 font-bold">MJ Panel</span> ارائه شده است. لطفاً از این ابزار فقط به صورت شخصی استفاده کنید.
        </p>
                    ...
                    <div class="flex flex-col gap-2 pt-4 lg:pt-0 border-t-2 lg:border-t-0 border-gray-300 dark:border-amoled-border justify-between">
                        <div class="flex flex-col flex-1">
                            ...
                            <div class="grid grid-cols-2 gap-2 mb-2 w-full">
                                ...
                                <a href="https://wa.me/971557256658" target="_blank" rel="noopener noreferrer" class="text-[11px] bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-2 py-2 rounded border border-green-200 dark:border-green-800 hover:bg-green-100 dark:hover:bg-green-900/50 transition font-black shadow-sm text-center whitespace-nowrap">واتساپ پشتیبانی</a>
                            </div>
        ...
        <div class="p-5 space-y-4">
            ...
            <div class="p-4 bg-gray-50 dark:bg-zinc-900/40 border border-gray-200 dark:border-amoled-border rounded-md">
                ...
                <p class="text-[10px] text-gray-500 dark:text-zinc-500 mb-3 leading-relaxed font-medium">
                    جستجو در منابع عمومی؛ به دلیل نیاز به تست کیفیت زمان‌بر است.
                </p>
...
<div id="support-modal" class="fixed inset-0 z-[105] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-red-500/50 rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <h3 class="font-black text-xl text-gray-900 dark:text-white mb-3">تماس با MJ Panel</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400 mb-6 leading-relaxed font-medium">
            برای پشتیبانی و هماهنگی فقط از واتساپ استفاده کن.
        </p>
        <div class="space-y-3">
            <a href="https://wa.me/971557256658" target="_blank" rel="noopener noreferrer" class="w-full py-3 bg-green-500/15 border-2 border-green-500 text-green-700 hover:bg-green-500/25 dark:border-green-400/60 dark:text-green-300 dark:hover:bg-green-500/15 font-bold rounded-md text-sm transition duration-300 shadow-sm flex items-center justify-center gap-2">
                <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M19.07 4.93A10 10 0 003.41 17.78L2 22l4.35-1.36A10 10 0 1019.07 4.93zM12 20a7.98 7.98 0 01-4.08-1.12l-.29-.17-2.58.81.84-2.51-.19-.32A8 8 0 1112 20zm4.62-5.84c-.25-.13-1.47-.73-1.7-.81-.23-.08-.39-.13-.56.13s-.64.81-.78.98-.29.19-.54.06a6.58 6.58 0 01-1.94-1.2 7.24 7.24 0 01-1.35-1.67c-.14-.24-.01-.37.12-.49.12-.12.27-.29.4-.44.13-.15.17-.25.26-.42.09-.17.05-.31-.02-.44-.07-.13-.56-1.35-.77-1.85-.2-.48-.4-.41-.56-.42h-.48c-.17 0-.44.06-.67.31-.23.26-.88.86-.88 2.1 0 1.23.9 2.42 1.03 2.59.13.17 1.77 2.7 4.28 3.79.6.26 1.07.42 1.44.54.61.2 1.17.17 1.61.1.49-.07 1.47-.6 1.68-1.18.21-.58.21-1.08.15-1.18-.06-.1-.23-.17-.48-.3z"/></svg>
                واتساپ: +971557256658
            </a>
        </div>
...
<div id="update-modal" class="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm opacity-0 pointer-events-none transition-all duration-300 ease-out">
    <div class="w-full max-w-md bg-white dark:bg-amoled-card border border-gray-200 dark:border-amoled-border rounded-md shadow-2xl overflow-hidden p-6 text-center transition-all transform duration-300 opacity-0 scale-95 ease-out">
        ...
        <div class="space-y-3">
            ...
            <a href="https://wa.me/971557256658" target="_blank" rel="noopener noreferrer" class="w-full py-3.5 bg-green-50 dark:bg-green-950/30 hover:bg-green-100 dark:hover:bg-green-900/50 text-green-700 dark:text-green-300 border border-green-300 dark:border-green-500 font-bold rounded-md text-sm transition duration-300 shadow-sm flex items-center justify-center gap-2">
...
const UPDATE_FIX = "constsCURRENT_VERSION='d.d.d'";
		async function checkForUpdates(isManual = false) {
            try {
                ...
                const res = await fetch('https://raw.githubusercontent.com/Jarahzadeh/MJ-Panel/main/Source.js?t=' + Date.now());
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            window.changeRefreshRate = function(val) {
                ...
                localStorage.setItem('mj_refresh_rate', ms);
                window.startRefreshInterval(ms);
                showToast('نرخ رفرش پـنـل تغییر کرد');
            };
            const savedRate = localStorage.getItem('mj_refresh_rate');
...
<div class="flex flex-col gap-4 mt-6 z-10">
    <div class="flex flex-wrap items-center gap-3 sm:gap-4 justify-center">
        <a href="https://wa.me/971557256658" target="_blank" rel="noopener noreferrer" class="flex items-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-full bg-green-500/20 hover:bg-green-500/30 border border-green-400/20 hover:border-green-300/30 transition-all duration-300 text-green-50 hover:text-white backdrop-blur-sm shadow-lg text-xs sm:text-sm font-medium group">
            <svg class="w-5 h-5 group-hover:scale-110 transition" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19.07 4.93A10 10 0 003.41 17.78L2 22l4.35-1.36A10 10 0 1019.07 4.93zM12 20a7.98 7.98 0 01-4.08-1.12l-.29-.17-2.58.81.84-2.51-.19-.32A8 8 0 1112 20zm4.62-5.84c-.25-.13-1.47-.73-1.7-.81-.23-.08-.39-.13-.56.13s-.64.81-.78.98-.29.19-.54.06a6.58 6.58 0 01-1.94-1.2 7.24 7.24 0 01-1.35-1.67c-.14-.24-.01-.37.12-.49.12-.12.27-.29.4-.44.13-.15.17-.25.26-.42.09-.17.05-.31-.02-.44-.07-.13-.56-1.35-.77-1.85-.2-.48-.4-.41-.56-.42h-.48c-.17 0-.44.06-.67.31-.23.26-.88.86-.88 2.1 0 1.23.9 2.42 1.03 2.59.13.17 1.77 2.7 4.28 3.79.6.26 1.07.42 1.44.54.61.2 1.17.17 1.61.1.49-.07 1.47-.6 1.68-1.18.21-.58.21-1.08.15-1.18-.06-.1-.23-.17-.48-.3z"/>
            </svg>
            واتساپ / WhatsApp: +971557256658
        </a>
    </div>
</div>
${COMMON_TOAST_HTML}
    <script>
        ...
        function getvIeesLink() {
			...
			const dynPath = encodeURIComponent("/stream/MJ_PANEL/" + (u.uuid ? u.uuid.split("-")[4] : "default"));
			var links = [];
            let flagEmoji = '🌐';
            ...
            ips.forEach(function(ip, ipIndex) {
                ports.forEach(function(portStr) {
					...
					var remark = "MJ | " + flagEmoji + " | " + u.username;
